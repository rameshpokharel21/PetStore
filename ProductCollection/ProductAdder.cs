...
public class ProductAdder
{
    ...
    {
        string typeName = UserInterface.GetProductType();
        string name = UserInterface.GetProductName(productLogic);
        decimal price = UserInterface.GetProductPrice();
        int quantity = UserInterface.GetProductQuantity();
        string description = UserInterface.GetProductDescription();

        if (typeName.Equals("catfood", StringComparison.OrdinalIgnoreCase))
        {
            double weightPounds = UserInterface.GetCatFoodWeightInPounds();
            bool kittenFood = UserInterface.IsCatFood();
        ...
        {
            int lengthInches = UserInterface.GetLeashLengthInInches();
            string? material = UserInterface.GetLeashMaterialType();
